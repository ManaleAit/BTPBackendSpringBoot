...
public class MaitreOuvrageService {
    ...
    public List<MaitreOuvrageDto> findAll() throws BureauEtudeException {
        ...
        return Optional.ofNullable(maitreOuvrageRepository.findAll())
                .map(MaitreOuvrageMapper::toDtos)
                .orElseThrow(() -> new BureauEtudeException(ExceptionCode.API_RESOURCE_NOT_FOUND, "Maitres Ouvrages not found"));
    }

    public void delete(Long id) {
    	
    	MaitreOuvrage maitreOuvrage=maitreOuvrageRepository.findById(id).get();
    ...
    public void save(MaitreOuvrageDto maitreOuvrageDto) {
        LOGGER.debug("START SERVICE save by id {}, name: {}", maitreOuvrageDto.getId(), maitreOuvrageDto.getDesignation());
    ...
    public MaitreOuvrageDto findById(Long id) throws BureauEtudeException {
        ...
        return Optional.ofNullable(maitreOuvrageRepository.findById(id))
                .map(v->MaitreOuvrageMapper.toDto(v.get()))
                .orElseThrow(() -> new BureauEtudeException(ExceptionCode.API_RESOURCE_NOT_FOUND, "maitre D'Ouvrage not found"));
    }
}
